...
describe('RoutesAdapter', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<Repository<RouteEntity>>(getRepositoryToken(RouteEntity));
  });
  ...
  describe('getAllRoutes', () => {
    it('should call repository', async () => {
      ...
      // When Then
      await expect(routesAdapter.getAllTravels()).resolves.toStrictEqual([new Travel('Tatooine', 'Dagobah', 4)]);
    });
  });
  ...
  describe('getTravelTime', () => {
    it('should return travel time', async () => {
      ...
      // When Then
      const result: number = await routesAdapter.getTravelTime('Tatooine', 'Dagobah');
