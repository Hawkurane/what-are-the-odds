...
export class Assert {
  static notBlank(fieldName: string, value: string): void {
    if (value.length === 0) throw MissingMandatoryValueException.forBlankValue(fieldName);
  }
...
class IntegerAsserter {
  ...
  min(minValue: number): IntegerAsserter {
    ...
    throw IntegerTooLowException.builder().setField(this.fieldName).setCurrentValue(this.value).setMinValue(minValue).build();
  }
}
