...
export class OddsDomainService {
  constructor(@Inject(Routes) private readonly routes: Routes, @Inject(Pathfinder) private readonly pathfinder: Pathfinder) {}

  async getOddsForEmpirePatrol(milleniumFalcon: MilleniumFalcon, empire: Empire): Promise<number> {
    const planets: string[] = await this.getAllPlanets();
    const travels: Travel[] = await this.routes.getAllTravels();
    const allPossiblePaths: string[][] = this.pathfinder.getAllPaths(milleniumFalcon.departure, milleniumFalcon.arrival, planets, travels);
    const allPossibleDailyLogs: string[][] = [];
    for (const path of allPossiblePaths) allPossibleDailyLogs.push(await this.getDailyTravelLog(milleniumFalcon, path));

    const allPossibleOddsFromDailyLogs: number[] = [];
    for (const dailyLog of allPossibleDailyLogs) allPossibleOddsFromDailyLogs.push(this.getOddsForSpecificPath(dailyLog, empire));
  ...
  getOddsForSpecificPath(path: string[], empire: Empire): number {
    ...
    for (const planetPatrol of empire.bountyHunters) if (path[planetPatrol.day - 1] === planetPatrol.planet) nbOfTimesRebelsMeetEmpire++;
  ...
  async getDailyTravelLog(milleniumFalcon: MilleniumFalcon, path: string[]): Promise<string[]> {
    ...
    while (position !== milleniumFalcon.arrival) {
      const travelTimeToNextPlanet: number = await this.routes.getTravelTime(path[day - 1], path[day]);
      if (milleniumFalconBattery < travelTimeToNextPlanet) {
        travelLog.push(position);
        milleniumFalconBattery = milleniumFalcon.autonomy;
      } else {
        for (let j = 0; j < travelTimeToNextPlanet - 2; j++) travelLog.push('Hyperspace');
