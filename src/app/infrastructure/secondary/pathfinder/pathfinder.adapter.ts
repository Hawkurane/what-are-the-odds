...
export class PathfinderAdapter implements Pathfinder {
  ...
  getAllPaths(
    ...
    return this.allPathsToSourceTarget(
      source,
      destination,
      this.edgeMatrixFromDomain(planets, edges),
    );
  }

  private edgeMatrixFromDomain(
    planets: string[],
    travels: Travel[],
  ): EdgeMatrix {
    ...
    for (const travel of travels)
      edgeMatrix[travel.departure].push(travel.destination);
    return edgeMatrix;
  }

  private allPathsToSourceTarget(
    source: string,
    destination: string,
    edgeMatrix: EdgeMatrix,
  ): string[][] {
